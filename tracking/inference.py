...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        #posDis = self.getPositionDistribution(gameState)
        #newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
        allPossible = util.Counter()
        for p in self.legalPositions:
            self.setGhostPosition(gameState, p)
            newPostDist = self.getPositionDistribution(gameState)
            for q in self.legalPositions:
                allPossible[q] += self.beliefs[p] * newPostDist[q] 
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in  range(self.numParticles):
            self.particles.append(self.legalPositions[ i%numPosition ])
    ...
    def observe(self, observation, gameState):
        ...
        allPossible = util.Counter() # a distribution (<=> weight)
        if noisyDistance == None:
            allPossible[self.getJailPosition()] = 1.0
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        legalPositions_product = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        random.shuffle(legalPositions_product)
        length = len(legalPositions_product)
        for i in range(self.numParticles) :
            positions = legalPositions_product[ i % length ] 
    ...
    def observeState(self, gameState):
        ...
        allPossible = util.Counter() # distribution
        for p in self.particles: # p is a tuple of positions
            weight = 1
            for i in range(self.numGhosts):
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                newPostDist = getPositionDistributionForGhost(gameState,
